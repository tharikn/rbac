...
let ProfileComponent$1 = class ProfileComponent {
    ...
    createRow(x) {
        return new FormGroup({
            phone: new FormControl(x?.phone_number ? x?.phone_number : '', [Validators.required, Validators.pattern(/\(\d{3}\) \d{3}-\d{4}/)]),
            HomePhone: new FormControl(x?.homephone ? x?.homephone : '', [Validators.required, Validators.pattern(/\(\d{3}\) \d{3}-\d{4}/)])
        });
    }
    addSerice() {
    ...
    getUserInfo() {
        this.profileService.getAllUserList(this.userid).subscribe((res) => {
            ...
            const providerData = patchValuedata.userroles[0];
            ...
            control.controls = [];
            providerData?.phone_numbers?.forEach(x => {
