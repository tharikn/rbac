...
class PageaccessComponent {
                ...
                pageLevelData.controls.forEach((control, index) => {
                    ...
                    if (originalEntry) {
                        control.patchValue({
                            ...
                            accessdata: originalEntry.validity
                        });
                    }
                    else {
                        control.patchValue({
    ...
    getPageAccessArray(pId, i, accessArray, pageName, setAccess, pageConfig) {
        accessArray.push(new FormGroup({
            ...
            pageAccessRead: new FormControl(this.pageAccessService.getPageReadFormAccess(setAccess)),
            ...
            validity: new FormControl((setAccess.conditions && setAccess.conditions.value) || '1'),
            condition: new FormControl(setAccess.conditions ? 'days' : 'always'),
            fallbackTo: new FormControl((setAccess.conditions && setAccess.conditions.fallbackTo) || 'n')
        }));
    }
    ...
    getAccessByAsset(accessArray) {
        for (const asset of this.mergedAsset) {
            ...
            accessArray.push(new FormGroup({
                ...
                fieldAccessRead: new FormControl(assetAccess?.r || false),
    ...
    changeFieldAccess(_event, i) {
        const accessArray = this.rbacForm.get('pageLevelData');
        const fieldLevelDataControl = this.rbacForm.get('fieldLevelData');
        if (this.rbacForm.get('fieldLevelData').value[i].fieldAccessCreate || this.rbacForm.get('fieldLevelData').value[i].fieldAccessRead || this.rbacForm.get('fieldLevelData').value[i].fieldAccessUpdate) {
            fieldLevelDataControl.controls[i].get('fieldAccessHide').patchValue(false);
            fieldLevelDataControl.controls[i].get('fieldAccessHide').disable();
        }
        else {
            fieldLevelDataControl.controls[i].get('fieldAccessHide').enable();
        }
        if (this.rbacForm.get('fieldLevelData').value[i].fieldAccessHide) {
            fieldLevelDataControl.controls[i].get('fieldAccessCreate').patchValue(false);
            fieldLevelDataControl.controls[i].get('fieldAccessCreate').disable();
            fieldLevelDataControl.controls[i].get('fieldAccessRead').patchValue(false);
            fieldLevelDataControl.controls[i].get('fieldAccessRead').disable();
            fieldLevelDataControl.controls[i].get('fieldAccessUpdate').patchValue(false);
            fieldLevelDataControl.controls[i].get('fieldAccessUpdate').disable();
        }
        else {
            fieldLevelDataControl.controls[i].get('fieldAccessCreate').enable();
            fieldLevelDataControl.controls[i].get('fieldAccessRead').enable();
            fieldLevelDataControl.controls[i].get('fieldAccessUpdate').enable();
        }
        ...
        if (pageIndex > -1) {
            accessArray.push(new FormGroup({
                ...
                accessdata: new FormControl(this.rbacForm.get('pageLevelData').value[pageIndex].accessdata),
