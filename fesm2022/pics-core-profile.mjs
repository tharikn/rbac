...
let ProfileComponent$1 = class ProfileComponent {
    ...
    createRow(x) {
        return new FormGroup({
            contactType: new FormControl(x?.contactType ? x?.contactType : '', [Validators.required]),
            contactNumber: new FormControl(x?.contactNumber ? x?.contactNumber : '', [Validators.required, Validators.pattern(/\(\d{3}\) \d{3}-\d{4}/)])
        });
    }
    addSerice() {
